...
class Media {
  ...
  onResize({ screen, viewport } = {}) {
    ...
    // Use viewport.height for both axes so aspect ratio stays consistent on all screen sizes
    this.plane.scale.y = (this.viewport.height * (900 * this.scale)) / this.screen.height;
    this.plane.scale.x = (this.viewport.height * (700 * this.scale)) / this.screen.height;
    if (isMobile) {
      // Scale down on mobile so multiple cards are visible at once
      this.plane.scale.y *= 0.6;
      this.plane.scale.x *= 0.6;
    }
    this.program.uniforms.uPlaneSizes.value = [this.plane.scale.x, this.plane.scale.y];
    this.padding = isMobile ? 0.5 : 2;
...
export default function CircularGallery({
  ...
  useEffect(() => {
    ...
    const font = `${computedStyle.fontWeight || "600"} ${computedStyle.fontSize || "28px"} ${computedStyle.fontFamily}`;
  ...
  return (
    <div
      ref={containerRef}
      className={`circular-gallery-container${className ? " " + className : ""}`}
      {...props}
    />
  );
}
